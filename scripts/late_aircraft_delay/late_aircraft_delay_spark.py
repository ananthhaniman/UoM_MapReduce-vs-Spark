...
spark = SparkSession.builder.appName("YearWiseLateAircraftDelay").getOrCreate()
...
df.write.format("parquet").partitionBy("_c1").save("s3://flights-delay/spark-output/late_aircraft_delay/DelayedFlights-updated-df")

# Read the saved Parquet file
df2 = spark.read.format("parquet").load("s3://flights-delay/spark-output/late_aircraft_delay/DelayedFlights-updated-df")
...
for i in range(5):
    ...
    result_path = f"s3://flights-delay/spark-output/late_aircraft_delay/iteration_{i}"
...
execution_times_path = "s3://flights-delay/spark-output/late_aircraft_delay/spark_late_aircraft_delay_time"
