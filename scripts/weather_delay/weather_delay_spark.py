...
spark = SparkSession.builder.appName("YearWiseWeatherDelay").getOrCreate()
...
df.write.format("parquet").partitionBy("_c1").save("s3://flights-delay/spark-output/weather_delay/DelayedFlights-updated-df")

# Read the saved Parquet file
df2 = spark.read.format("parquet").load("s3://flights-delay/spark-output/weather_delay/DelayedFlights-updated-df")
...
for i in range(5):
    ...
    result_path = f"s3://flights-delay/spark-output/weather_delay/iteration_{i}"
...
execution_times_path = "s3://flights-delay/spark-output/weather_delay/spark_weather_delay_time"
